...
function confirmRemoveUser(user) {
    const confirmation = window.confirm(`Are you sure you want to remove the user "${user.username}"?`);
    if (confirmation) {
        removeUser(user);
    }
}
...
function confirmRemoveBook(book) {
    const confirmation = window.confirm(`Are you sure you want to remove the book "${book.title}" by ${book.author}?`);
    if (confirmation) {
        removeBook(book);
    }
}
...
function removeBook(book) {
    const books = JSON.parse(localStorage.getItem('books_info')) || [];
    const index = books.findIndex(b => b.title === book.title && b.author === book.author);
